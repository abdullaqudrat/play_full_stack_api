...
const findJoin = (favoriteId, playlistId) => database('favorites')
    ...
    .where({ favorite_id: favoriteId, playlist_id: playlistId })
    .select()
    .limit(1)
  
const deleteJoin = (favoriteId, playlistId) => database('playlists_favorites')
    .where({ favorite_id: favoriteId, playlist_id: playlistId })
    .del()

const insertFavorite = (playlistId, favoriteId) => database('playlists_favorites')
    .insert({ favorite_id: favoriteId, playlist_id: playlistId })
